  ...
  <div>
    <div class="filter-container">
      ...
      <el-button
        ...
        type="primary"
        icon="el-icon-edit"
        @click="$router.push({ path: '/video/edit' })"
      >
        添加
      </el-button>
    </div>
...
<script>
import { reqVidsList, changeVideoState } from "@admin/api";
...
export default {
  ...
  methods: {
    getTableData() {
      reqVidsList({ pageNum: 1, pageSize: 20 })
        .then((res) => {
          if (res.status === 200) {
            this.initData = this.tableData = res.data.vidsList;
          }
        })
        .catch(() => {
          this.$notify({
