  ...
  <div>
    <div class="filter-container">
      ...
      <el-button
        ...
        type="primary"
        icon="el-icon-edit"
        @click="dialogUploadFormVisible = true"
      >
        添加
      </el-button>
    </div>
    ...
    >
      ...
      <el-table-column property="insituContent" align="left" label="内容">
      </el-table-column>
      <el-table-column
    ...
    <el-dialog title="制度制定" :visible.sync="dialogUploadFormVisible">
      <el-form :model="uploadContent" ref="uploadContent" :rules="rules">
        ...
        >
          <el-input
            v-model="uploadContent.subContent"
            autocomplete="off"
          ></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="formValidate('uploadContent')"
          >确 定</el-button
        >
      </div>
    </el-dialog>
    ...
    <el-dialog title="修改制度" :visible.sync="dialogUpdateFormVisible">
      <el-form :model="updateContent">
        ...
        <el-form-item label="修改内容" :label-width="formLabelWidth">
          <el-input
            v-model="updateContent.subContent"
            autocomplete="off"
          ></el-input>
        </el-form-item>
      </el-form>
...
<script>
import { reqRegulations, uploadRegulation, updateRegulation, deleteRegulation} from "@admin/api";
...
export default {
  ...
  methods: {
    ...
    handleFilter() {
      const _title =
        this.listQuery.title === "" ? "" : this.listQuery.title.toLowerCase();
      const _time =
        this.listQuery.time === ""
          ? ""
          : this.listQuery.time.toLocaleDateString().replace(/\//g, "-");
      const isBlank = !(_title || _time);

      const filterResult = this.initData.filter((item) => {
        return (
          isBlank ||
          (_title && item.insituContent.toLowerCase().includes(_title)) ||
          (_time && item.lastUpdateTime.includes(_time))
        );
      });
      ...
      reqRegulations()
        .then((res) => {
          if (res.data.status === 200)
            this.initData = this.tableData = res.data.regulations;
        })
        .catch(() => {
          this.$$notify({
            type: "error",
