      ...
      >
        ...
        <el-table-column width="250" label="组别名称">
          <template slot-scope="{ row, $index }">
            <template v-if="row.edit">
              <el-input
                v-model="tableData[$index].groupName"
                class="edit-input"
                size="small"
              />
              <el-button
        ...
        <el-table-column min-width="200" show-overflow-tooltip label="简介">
          <template slot-scope="{ row, $index }">
            <template v-if="row.edit">
              <el-input
                v-model="tableData[$index].groupIntro"
                type="textarea"
                autosize
                class="edit-input"
              />
          ...
          <transition name="el-zoom-in-center">
            <div class="image-preview" v-show="form.pre_img">
              <div class="image-preview-wrapper">
                <el-image :src="form.pre_img" class="image"> </el-image>
                <div class="image-preview-action">
                  <i
                    class="el-icon-delete"
                    @click="form.image = form.pre_img = ''"
                  ></i>
                </div>
              </div>
            </div>
  ...
  methods: {
      ...
      reqAllGroups()
        .then((res) => {
          if (res.data.status === 200) {
            console.log(res);
            this.loading = false;
            this.initData = this.tableData = res.data.groups.map((v) => {
              this.$set(v, "edit", false); // https://vuejs.org/v2/guide/reactivity.html
              v.originalName = v.groupName;
              v.originalIntro = v.groupIntro; //  will be used when user click the cancel botton
              v.groupIsenabled == "1"
                ? (v.groupIsenabled = true)
                : (v.groupIsenabled = false); // 解决服务器标识符为数字 switch要求为布尔问题
    ...
    handleFilter() {
      ...
      const filterResult = this.initData.filter((item) => {
        return (
          !_title || (_title && item.groupName.toLowerCase().includes(_title))
        );
      });
    ...
    changeStatus(newV, row, index) {
      ...
      changeGroupState({ id: row.id, groupIsenabled: status })
        .then((res) => {
          if (res.status === 200) {
            this.$notify({ type: "success", message: "修改成功" });
          }
