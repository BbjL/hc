<template>
  <div class="resource-page">
    ...
    <div class="card-container" v-if="resource.length > 0">
      <el-row>
        <el-col
          class="el-col-lg-6 el-col-md-6 el-col-sm-12"
          v-for="(item, index) in resource"
          :key="index"
          style="padding: 0 7px; margin-top: 20px"
        >
          <el-card :body-style="{ padding: '0px' }">
            <el-image
              src="@/assets/images/client/settings/car.jpg"
              class="image"
            >
            </el-image>
            <div style="padding: 14px" class="card-content">
              ...
              <div class="bottom">
                <p
                  :class="item.resDetail.length > 50 ? 'mutiline_ellipsis' : ''"
                >
                  {{ item.resDetail }}
                </p>
              </div>
    ...
    <el-dialog title="资源信息" :visible.sync="dialogDetailVisible">
      <div class="dialog-content">
        ...
        <el-image src="" class="image"> </el-image>
        <h5 style="margin-top: 20px">设备描述：</h5>
        <p>{{ detail.content }}</p>
      </div>
      <div slot="footer" class="dialog-footer">
        <el-button @click="dialogDetailVisible = false" type="primary"
          >取 消</el-button
        >
      </div>
    </el-dialog>
          ...
          <transition name="el-zoom-in-center">
            <div class="image-preview" v-show="form.previewUrl">
              <div class="image-preview-wrapper">
                <el-image :src="form.previewUrl" class="image"> </el-image>
                <div class="image-preview-action">
                  <i
                    class="el-icon-delete"
                    @click="form.imageData = form.previewUrl = ''"
                  ></i>
                </div>
              </div>
            </div>
        ...
        >
          ...
          <transition name="el-zoom-in-center">
            <div class="image-preview" v-show="updateForm.previewUrl">
              <div class="image-preview-wrapper">
                <el-image :src="updateForm.previewUrl" class="image">
                </el-image>
                <div class="image-preview-action">
                  <i
                    class="el-icon-delete"
                    @click="updateForm.previewUrl = ''"
                  />
                </div>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    updateSetView(item) {
      ...
      this.updateForm.previewUrl =
        this.$store.state.BASE_URL +
        this.$store.state.reso_imagesUrl +
        target.id;
    },
...
.card-container .card-content .more {
  position: absolute;
  top: -19px;
  right: 10px;
}
.card-container .card-content .card-tit {
  font-size: 14px;
  font-weight: 600;
  color: #303133;
}
.card-container .card-content .bottom {
  height: 60px;
  overflow: hidden;
  color: #606266;
}
.card-container .card-content .bottom > p {
  letter-spacing: 1px;
}
.dialog-content {
  max-height: 400px;
...
.image-preview .image-preview-wrapper .el-image {
  height: 100%;
}
.image-preview .image-preview-wrapper .el-image img {
  height: 100%;
}
