    ...
    <article>
      ...
      <header class="article-header">
        ...
        <div class="basic-info">
          <span><i class="fa fa-user-circle"></i>负责人：{{info.leader}}</span>
          <span><i class="fa fa-calendar"></i>开始时间：{{info.date}}</span>
        </div>
...
import RecentItem from "@/components/RecentItem/index";
import { filterBraed } from "@/utils/filters";
import { reqProjInfo, reqProjsRecent } from "@/api/index";
...
export default {
  ...
  beforeMount() {
    ...
    reqProjInfo({ id }).then((res) => {
      console.log(res);
      this.info = res.data.projs;
    });

    reqProjsRecent().then(res => {
      console.log(res);
      this.recent = res.data.projs;
    })
  },
  methods: {
    handleSubmit() {
